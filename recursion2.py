...
#  Description: Answers to recursion exercise 2
...
def groupSum(start, nums, target):
    if start>=len(nums):
        ...
        return groupSum(start+1,nums,target-num[start]) or groupSum(start+1,nums,target)
...
def groupSum6(start, nums, target):
    if start>=len(nums):
        ...
        return groupSum6(start+1,nums,target-num[start])
    else:
        return groupSum6(start+1,nums,target-num[start]) or groupSum6(start+1,nums,target)
...
def groupNoAdj(start, nums, target):
    if start>=len(nums):
        return target==0
    elif num[start]==target:
        return True
    elif start==len(nums)-1:
        return target==nums[start]
    else:
        groupNoAdj(start+2,nums,target-nums[start]) or groupNoAdj(start+1,nums,target)
    ...
    if start>=len(nums):
        ...
        for i in range(start+1,len(nums)):
            if nums[start]==nums[i]:
                total+=nums[i]
                count+=1
        return groupSumClump(start+count,nums,target-total) or groupSumClump(start+count,nums,target)
...
def splitArray(nums):
    ...
    listB=[]
    sum_num=sum(nums)
    splitArrayHelper(nums,numslistA,listA,0)
    for i in listB:
        if sum_num-i==i:
            return True
        else:
...
def splitArrayHelper(numbers,list_a,list_b,index):
    if index==len(numbers):
        sum_numbers=sum(list_a)
        list_b.append(sum_numbers)
    else:
        list_c=list_a[:]
        list_a.append(numbers[index])
        splitArrayHelper(numbers,list_a,list_b,index+1)
        splitArrayHelper(numbers,list_c,list_b,index+1)
...
def splitOdd10(nums):
    lista=[]
    return splitOdd10Helper(nums,lista,0)
  
def splitOdd10Helper(numbers,listay,start):
    if start==len(numbers):
        if sum(listay)%10==0 and (sum(numbers)-sum(listay))%2==1:
            return True
        else:
            return False
    else:
        listnew=listay[:]
        listay.append(numbers[start])
        return splitOdd10Helper(numbers,listay,start+1) or splitOdd10Helper(numbers,listnew,start+1)
...
def split53(nums):
    sum3=0
    sum5=0
    return split53Helper(nums,sum3,sum5,0)

def split53Helper(numbers,sumof3,sumof5,index):
    if index==len(numbers):
        if sumof3==sumof5:
            return True
        else:
        ...
        if numbers[index]%5==0:
            return split53Helper(numbers,sumof3,sumof5+numbers[index],index+1)
        if numbers[index]%3==0:
            return split53Helper(numbers,sumof3+number[index],sumof5,index+1)
        else:
            return split53Helper(numbers,sumof3,sumof5+numbers[index],index+1) or split53Helper(numbers,sumof3+number[index],sumof5,index+1)
